...
> Source: https://help.qsys.com/Content/Schematic_Library/Attero_Tech_extension_unDX2IOplus.htm

# Attero Tech unDX2IO+

Use this extension to connect to and control a unDX2IO+ Dante/AES67 network audio wallplate (Attero Tech by QSC).

**Note:** See the [unDX2IO+ product page](https://www.qsys.com/systems/products/q-sys-ecosystem/products-peripherals-accessories/attero-tech/at-wall-mount-network-audio-interfaces/undx2io/) on the QSC website for product information and documentation.

[Compatibility](javascript:void(0))

The extension is compatible with Q-SYS Designer version 8.4 and above and Attero Tech firmware version 4.6.0 and above.
...
For channels 1 and 2 only, selects the gain level for the input. Options are:
...
For channels 1 and 2 only, selects the state of phantom power for the input. If the button is grey, the phantom power is inactive. If itâs orange, the phantom power is active. Click the button to toggle between states.

#### Input Pad

For channels 3 and 4 only, selects the sensitivity for the input. Options are:
