# Lua 5.3 Reference Manual - Operating System Facilities

> Source: https://help.qsys.com/Content/Control_Scripting/Lua_5.3_Reference_Manual/Standard_Libraries/7_-_Operating_System_Facilities.htm
...
**Tip:** Because the output from `os.clock()` is platform-dependent, we recommend using `os.date()` instead.
...
(see the [`os.time`](#pdf-os.time) function for a description of this value).
...
`year`, `month` (1â12), `day` (1â31),
`hour` (0â23), `min` (0â59), `sec` (0â61),
...
(where the times are values returned by [`os.time`](#pdf-os.time)).
...
For a description of these fields, see the [`os.date`](#pdf-os.date) function.
...
[`os.date`](#pdf-os.date) and [`os.difftime`](#pdf-os.difftime).
