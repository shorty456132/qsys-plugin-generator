...
> Source: https://help.qsys.com/Content/Schematic_Library/lcqln_ir_output.htm

# IR Output (QIO-IR1x4)

The IR Output component represents an IR Output 1-4 connection on a [QIO-IR1x4](../Hardware/IO_Expanders/QIO-IR1x4.htm) device. Use the component to monitor IR activity, set the drive strength of the IR signal, and view the command code actively being sent.
...
Connect this pin to the IR Driver Out pin of the [IR Driver](ir_driver.htm) component or to the IR Input Out pin of the [IR Input (QIO-IR1x4)](lcqln_ir_input.htm) component. You can wire multiple IR sources to the component by adjusting the IR Source Count property. See [Properties](#Properti).

### Output Pins

#### Serial Port

This pin only appears when Serial RS-232 is selected for the Data Format property. Wire this pin to a downstream Scripting Component. To learn more, see [SerialPorts](../Control_Scripting/Using_Lua_in_Q-Sys/SerialPorts.htm).
...
When set to 'Yes', you can test IR output functionality with a Transmit Code control in the control panel. The default is 'No'.

#### Data Format

Select whether the IR Output communicates in IR format (default) or Serial RS-232. If Serial RS-232 is selected, a Serial Port output pin appears for connection to downstream Scripting Components. To learn more, see [SerialPorts](../Control_Scripting/Using_Lua_in_Q-Sys/SerialPorts.htm).

**Note:** RS-232 Mode supports TX only.
...
Select whether the IR Output is assigned to the Core (default) or an I/O Frame in your design. For I/O Frame, you must also type the Name of the device in your design.
...
This LED turns on whenever the IR Output port is actively sending information.

#### Drive Strength

Select the strength of the signal from the QIO-IR1x4 IR Output: Low (default), Medium, or High. You might need to increase the strength to overcome longer distances between the IR emitter and the receiving device.

#### Active Code

This is the IR code currently being transmitted from the IR Output.
...
Click to reset the TX Bytes counter to zero.
...
| Drive Strength | - | Low  Medium  High | - | Input/Output |
| Port Activity | 0  1 | false  true | 0  1 | Output |
| Reset[1](#Serial) | (trigger) | | | Input/Output |
| Transmit Bytes[1](#Serial) | (text) | | | Output |

###### 1. Visible in Serial RS-232 Mode only.
