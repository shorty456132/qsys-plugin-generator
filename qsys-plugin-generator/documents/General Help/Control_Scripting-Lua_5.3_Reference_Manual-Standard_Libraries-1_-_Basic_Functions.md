# Lua 5.3 Reference Manual - Basic Functions

> Source: https://help.qsys.com/Content/Control_Scripting/Lua_5.3_Reference_Manual/Standard_Libraries/1_-_Basic_Functions.htm
...
* **"`setpause`":** 
  ...
  the collector (see [Garbage Collection](../2_-_Basic_Concepts.htm#5)).
...
holds the global environment (see [Environments and the Global Environment](../2_-_Basic_Concepts.htm#2)).
...
will iterate over the keyâvalue pairs
(`1,t[1]`), (`2,t[2]`), ...,
...
the `_ENV` variable (see [Environments and the Global Environment](../2_-_Basic_Concepts.htm#2)).
However,
when you load a binary chunk created from a function (see [`string.dump`](3_-_String_Manipulation.htm#pdf-string.dump)),
...
See function [`next`](#pdf-next) for the caveats of modifying
...
using the  [See function [`next`](#pdf-next) for the caveats of modifying the table during its traversal.](#pdf-tostring)  function to convert each argument to a string.
`print` is not intended for formatted output,
but only as a quick way to show a value,
for instance for debugging.
For complete control over the output,
use [`string.format`](3_-_String_Manipulation.htm#pdf-string.format).
...
according to the lexical conventions of Lua (see [Lexical Conventions](../3_-_The_Language.htm#1)).
...
This function is similar to [`pcall`](#pdf-pcall),
