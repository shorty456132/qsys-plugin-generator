...
> Source: https://help.qsys.com/Content/Schematic_Library/io_card_flex_in_hdio.htm

# Flex In (QIO Series)

The Flex Channel feature provides four audio channels (QIO-FLEX4A) or eight audio channels (QIO-24f) that can be individually switched between line inputs and line outputs. The Flex In component provides the control and selection of the mic/line-level inputs on the device. The Flex Input component converts the analog signal to digital and provides software controls before and after the convertor. Connections to a single channel are made using a three-terminal 3.5mm Euro style connector.
...
#### Input 1-4 (QIO-FLEX4A) / Channel 1-8 (QIO-24f)
...
Multicolored LED indicating the status of the I/O-8 Flex In component. This LED matches the Status text field.
