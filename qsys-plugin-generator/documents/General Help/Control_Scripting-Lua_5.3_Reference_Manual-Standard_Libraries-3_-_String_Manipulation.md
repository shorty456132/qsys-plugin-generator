...
> Source: https://help.qsys.com/Content/Control_Scripting/Lua_5.3_Reference_Manual/Standard_Libraries/3_-_String_Manipulation.htm
...
following the same rules of function [`string.sub`](#pdf-string.sub).
...
so that a later [`load`](1_-_Basic_Functions.htm#pdf-load) on this string returns
...
`pattern` (see [Patterns](#3.1)) in the string `s`.
...
it is converted to one following the same rules of [`tostring`](1_-_Basic_Functions.htm#pdf-tostring).
...
**QSC Note:** for a useful reference to the formatting parameters used in Lua's C-like string.format method, follow the formatting advice [here](http://www.cplusplus.com/reference/cstdio/printf/).
...
returns the next captures from `pattern` (see [Pattern](#3.1))
over the string `s`.
...
occurrences of the `pattern` (see [Pattern](#3.1)) have been
...
Looks for the first *match* of
`pattern` (see [Pattern](#3.1)) in the string `s`.
...
according to the format string `fmt` (see [Format Strings](#3.2)).

---

### `string.packsize (fmt)`

Returns the size of a string resulting from [`string.pack`](#pdf-string.pack)
with the given format.
The format string cannot have the variable-length options
'`s`' or '`z`' (see [Format Strings](#3.2)).
...
Returns the values packed in string `s` (see [`string.pack`](#pdf-string.pack))
according to the format string `fmt` (see [Format Strings](#3.2)).
...
and [`string.match`](#pdf-string.match).
...
### 3.2 â Format Strings for Pack and Unpack

The first argument to [`string.pack`](#pdf-string.pack),
[`string.packsize`](#pdf-string.packsize), and [`string.unpack`](#pdf-string.unpack)
is a format string,
...
(options "`xX <=>!`"),
each option corresponds to an argument (in [`string.pack`](#pdf-string.pack))
or a result (in [`string.unpack`](#pdf-string.unpack)).

For options "`!n`", "`sn`", "`in`", and "`In`",
`n` can be any integer between 1 and 16.
All integral options check overflows;
[`string.pack`](#pdf-string.pack) checks whether the given value fits in the given size;
[`string.unpack`](#pdf-string.unpack) checks whether the read value fits in a Lua integer.
