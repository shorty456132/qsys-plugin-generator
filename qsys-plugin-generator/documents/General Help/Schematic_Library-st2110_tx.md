# ST 2110 Audio Transmitter

> Source: https://help.qsys.com/Content/Schematic_Library/st2110_tx.htm

# ST 2110 Audio Transmitter

The Q-SYS ST 2110 Audio Transmitter enables the transmission of SMPTE ST 2110-compliant audio streams with optional network redundancy over IP networks. It supports network redundancy and can be configured to use either manual settings or NMOS for automatic discovery and connection management. QSC only supports ST 2110-30, Conformance Level A at this time.
...
By default, the ST 2110 Transmitter is set to 8 audio input channels. You can choose between 1 through 8.
...
*Manual*: Manually configure the network settings for the ST 2110 Audio Transmitter.

*NMOS*: The transmitter will use the Networked Media Open Specifications (NMOS) protocol for automatic discovery and connection management.
...
Measures the transmitted audio peak output level, from -120 dB to 20 dB.
...
Activates or deactivates the Primary and Secondary Leg of the Tx.
...
Activates or deactivates the Primary/Secondary Leg of the Tx.
...
Sets the IP address to which the media stream is sent.

#### Destination Port

Specifies the port number to which the media stream is sent.

#### Source Port

Indicates the port from which the media stream is sent.

#### Source IP

*(Read-only field)*: Indicates the IP address from which the media stream is sent.

#### Interface

*(Read-only field)*: Indicates the network interface used for sending the media stream.
...
The Details section displays cumulative information for audio being sent from the source.
...
*(Read-only field)*: Displays the SDP associated with the stream being transmitted.
...
| Connection | | | | |
| Additional Network Tx Latency | None  Extra 1 ms  Extra 2 ms  Extra 5 ms | | | Input / Output |
| Flow Label | (text) | | | Input / Output |
| Global Enable | 0  1 | disabled  enabled | 0  1 | Input / Output |
| Media Clock Offset | 0 to *nnn* | 0 to *nnn* | 0 to 1.00 | Input / Output |
| Media Type | L16  L24 | L16  L24 | N/A | Input / Output |
...
| Raw SDP | (text) | | | Output |
| Primary Leg | | | | |
| Destination IP | *nnn*.*nnn*.*nnn*.*nnn* | | | Input / Output |
...
| Source IP | *nnn*.*nnn*.*nnn*.*nnn* | | | Output |
| Source Port | 1 to 65535 | 1 to 65535 | 0.00 to 1.00 | Input / Output |
